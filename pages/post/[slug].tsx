      ...
      alt=""
                ...
                serializers={
                    {
                        ...
                        l1: ({ children }: any) => ( <li className="ml-4 list-disc"> {children}</li>
                        ),
       ...
       {submitted ?(
           <div className="flex flex-col py-10 my-10 bg-yellow-500 text-white max-w-2xl mx-auto">
               <h3 className="text-3xl font-bold">Thank you for submnitting</h3>
               <p>It'll appear below once approved</p>
           </div>
       ): (
        ...
        <h3 className="text-sm text-yellow-500"> Enjoy this article?</h3>
            ...
            <label className="block mb-5">
                ...
                {...register("email", {required: true})}
                 className="shadow border rounded py-2 px-3 form-input mt-1 block w-full ring-yellow-500 outline-none focus:ring" type="email" placeholder="John Doe"  />
            </label>
            <label className="block mb-5">
                <span className="text-gray-700">Comment</span>
                <textarea 
                {...register("comment", {required: true})}
                className="shadow border rounded py-2 px-3 form-textarea mt-1 block w-full ring-yellow-500 outline-none focus:ring" type="text" placeholder="John Doe" rows={8} />
            </label>
            <div className="flex flex-col p-5">
                ...
                <input type="submit"  className="bg-yellow-500 hover:bg-yellow-400 focus:shawdow-outline focus:outline-none text-white font-bold py-2 px4 rounded cursor-pointer"/>
            </div>
        </form>
         )}
         {/* comments */}
...
export const getStaticProps: GetStaticProps = async({params}) => {
    const query = `*[_type == "post" && slug.current == $slug][0]{
        ...
        return {
            ...
            revalidate: 60, //update old cache in 60 seconds
        };
};
